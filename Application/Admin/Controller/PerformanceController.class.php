...
class PerformanceController extends AdminbaseController{
	...
	public function branchAchieve(){
		$insurance_type = I('post.insurance_type');
		$insurance_type = isset($insurance_type) ? $insurance_type : 1 ;
		...
		->where('insurance_type="'.$insurance_type.'" and policy_status=1 and FROM_UNIXTIME(insured_date,"%Y-%m")=DATE_FORMAT(NOW(),"%Y-%m")')
		...
		->where('insurance_type="'.$insurance_type.'" and policy_status=1 and QUARTER(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=QUARTER(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
		->group('branch_shop_number')//根据分公司分组
		...
		/******************分公司昨日本半年业绩及排名start***************/
		$res4 = $tmodel
		->where('insurance_type="'.$insurance_type.'" and policy_status=1 AND
                            if(
		...
		->where('insurance_type="'.$insurance_type.'" and policy_status=1 and YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
	...
	public function standAchiev(){
		...
		$stand_res2 = $tmodel->where('insurance_type="'.$insurance_type.'" and policy_status=1 and FROM_UNIXTIME(insured_date,"%Y-%m")=DATE_FORMAT(NOW(),"%Y-%m")')
		...
		$stand_res3 = $tmodel->where('insurance_type="'.$insurance_type.'" and policy_status=1 and QUARTER(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=QUARTER(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
		...
		/*********************标准店昨日本季度业绩及排名end******************/
		/*********************标准店昨日本半年业绩及排名start******************/
		$stand_res4 = $tmodel->where('insurance_type="'.$insurance_type.'" and policy_status=1 AND
                            if(
                                MONTH(DATE_FORMAT(NOW(),"%Y-%m-%d"))+0 <= 6,
                                YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))
                                    AND MONTH(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))+0 <= 6,
		...
		/*********************标准店昨日本年业绩及排名start******************/
		//首期，保单类型-寿险-insurance_type=1，保单状态-policy_status（有效），时间-insured_date-(当天，当月，本季度，本半年，本年)，分公司代码=post获取到的分公司代码
		$stand_res5 = $tmodel->where('insurance_type="'.$insurance_type.'" and policy_status=1 and YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
...
>>>>>>> 2041930c19fa6757f610c22ab18c3fd9ef235de5
	...
	public function teamcore(){
		...
		{
			$param = I('post.');
			$insurance_type = $param['insurance_type'];				//所属保单类型编号
			...
			//空值搜索，不查询
			if(empty($insurance_type) && empty($branch_shop_number) && empty($flag_shop_number) && empty($stand_shop_number))
			{
				$this->display();
				return false;
			}
			//查找分公司     公司不为空-旗舰店为空-标准店为空
			else if(!empty($insurance_type) && !empty($branch_shop_number) && empty($flag_shop_number) && empty($stand_shop_number))
			{
				...
				->where('policy_status=1 AND FROM_UNIXTIME(insured_date,"%Y-%m-%d")=DATE_FORMAT(NOW(),"%Y-%m-%d") AND branch_shop_number="'.$branch_shop_number.'" AND insurance_type="'.$insurance_type.'"')
				->group('branch_shop_number')//根据旗舰店分组
				//                    ->order('day_pay desc')//根据保险保费倒序排名
				->select();

				/*搜索分公司昨日排名业绩表*/
				$branchInfo = $branchModel->where('branch_id = "'.$branch_shop_number.'" AND ranking_type = "'.$insurance_type.'"')->select();
				//查找分公司的名称及规模人力
				$member = $mmodel
				->alias('m')
				...
				->where('branch_shop_number = '.$branch_shop_number.' AND insurance_type='.$insurance_type.' AND insurance_type = 1 AND policy_status=1 AND FROM_UNIXTIME(insured_date,"%Y-%m")=DATE_FORMAT(NOW(),"%Y-%m")')
				->select();
				//查找当季度开单人力
				$policy_quar = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('branch_shop_number = '.$branch_shop_number.' AND insurance_type='.$insurance_type.' AND policy_status=1 AND QUARTER(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=QUARTER(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
				->select();
				//查找本半年开单人力
				$policy_half = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('branch_shop_number = '.$branch_shop_number.' AND insurance_type='.$insurance_type.' AND policy_status=1 AND if(
                                MONTH(DATE_FORMAT(NOW(),"%Y-%m-%d"))+0 <= 6,
                                YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))
                                    AND MONTH(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))+0 <= 6,
                                YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))
				...
				$policy_year = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('branch_shop_number = '.$branch_shop_number.' AND insurance_type='.$insurance_type.' AND policy_status=1 AND YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
				->select();
				//查找机构目标
				$branchgoals = $ogmodel
			...
			//查找旗舰店，查询所有分公司下的旗舰店首期寿险业绩
			else if(!empty($insurance_type) && !empty($branch_shop_number) && !empty($flag_shop_number) && empty($stand_shop_number))
			{
				$flagModel = D('insu_flag_performance');//实例化旗舰店昨日排名表
				//当日业绩
				//首期，保单类型-寿险-insurance_type=1，保单状态-policy_status（有效），时间-insured_date-(当天，当月，本季度，本半年，本年)，分公司代码=post获取到的分公司代码
				$res = $tmodel->where('policy_status=1 AND FROM_UNIXTIME(insured_date,"%Y-%m-%d")=DATE_FORMAT(NOW(),"%Y-%m-%d") AND branch_shop_number="'.$branch_shop_number.'" AND flag_shop_number="'.$flag_shop_number.'" AND insurance_type="'.$insurance_type.'"' )
				...
				/*搜索旗舰店昨日排名业绩表*/
				$flagInfo = $flagModel
				->where('flag_id = "'.$flag_shop_number.'" AND ranking_type = "'.$insurance_type.'"')
				->select();
				//查找旗舰店的名称及规模人力
				$member = $mmodel
				->alias('m')
				...
				$policy_mon = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('flag_shop_number = '.$flag_shop_number.' AND insurance_type = '.$insurance_type.' AND policy_status=1 AND FROM_UNIXTIME(insured_date,"%Y-%m")=DATE_FORMAT(NOW(),"%Y-%m")')
				->select();
				//查找当季度开单人力
				$policy_quar = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('flag_shop_number = '.$flag_shop_number.' AND insurance_type = '.$insurance_type.' AND policy_status=1 AND QUARTER(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=QUARTER(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
				->select();
				//查找本半年开单人力
				$policy_half = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('flag_shop_number = '.$flag_shop_number.' AND insurance_type = '.$insurance_type.' AND policy_status=1 AND if(
                                MONTH(DATE_FORMAT(NOW(),"%Y-%m-%d"))+0 <= 6,
                                YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))
                                    AND MONTH(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))+0 <= 6,
                                YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))
				...
				$policy_year = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('flag_shop_number = '.$flag_shop_number.' AND insurance_type = '.$insurance_type.' AND policy_status=1 AND YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
				->select();
				//查找机构目标
				$branchgoals = $ogmodel
				->field('shop_code,insurance_target')
			...
			//查找标准店，查询所在分公司下的旗舰店下的标准店首期寿险业绩
			else if(!empty($insurance_type) && !empty($branch_shop_number) && !empty($flag_shop_number) && !empty($stand_shop_number))
			{
				$standModel = D('insu_stand_performance');
				//当日业绩
				//首期，保单类型-寿险-insurance_type=1，保单状态-policy_status（有效），时间-insured_date-(当天，当月，本季度，本半年，本年)，分公司代码=post获取到的分公司代码
				$res = $tmodel->where('policy_status=1 AND FROM_UNIXTIME(insured_date,"%Y-%m-%d")=DATE_FORMAT(NOW(),"%Y-%m-%d") AND branch_shop_number="'.$branch_shop_number.'" AND flag_shop_number="'.$flag_shop_number.'" AND standard_shop_number="'.$stand_shop_number.'" AND insurance_type="'.$insurance_type.'"')
				...
				->where('sand_id = "'.$stand_shop_number.'" AND ranking_type = "'.$insurance_type.'"')
				->select();
				//查找标准店的名称及规模人力
				$member = $mmodel
				->alias('m')
				...
				$policy_mon = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('standard_shop_number = '.$stand_shop_number.' AND insurance_type = "'.$insurance_type.'" AND policy_status=1 AND FROM_UNIXTIME(insured_date,"%Y-%m")=DATE_FORMAT(NOW(),"%Y-%m")')
				->select();
				//查找当季度开单人力
				$policy_quar = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('standard_shop_number = '.$stand_shop_number.' AND insurance_type = "'.$insurance_type.'" AND policy_status=1 AND QUARTER(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=QUARTER(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
				->select();
				//查找本半年开单人力
				$policy_half = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('standard_shop_number = '.$stand_shop_number.' AND insurance_type = "'.$insurance_type.'" AND policy_status=1 AND if(
                                MONTH(DATE_FORMAT(NOW(),"%Y-%m-%d"))+0 <= 6,
                                YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))
                                    AND MONTH(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))+0 <= 6,
                                YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))
				...
				$policy_year = $tmodel
				->field('count( distinct salesman_number) as policy_num')
				->where('standard_shop_number = '.$stand_shop_number.' AND insurance_type = "'.$insurance_type.'" AND policy_status=1 AND YEAR(FROM_UNIXTIME(insured_date,"%Y-%m-%d"))=YEAR(DATE_FORMAT(NOW(),"%Y-%m-%d"))')
				->select();
				//查找机构目标
				$branchgoals = $ogmodel
				->field('shop_code,insurance_target')
	...
	public function branchAjax()
	{
		...
		{
			echo '<option value="">请选择'.$string.'</option>';
		}
	}
}