...
/************************续期结算保费********************************************/
		...
		$five_time_result = $renewal_time ->table("renew as r")
										  ...
										  ->field("r.insurance_num,r.renew_count,r.five_agency_fee,r.five_time,i.insurance_premium,i.real_insurance_premium,i.insured_date,i.provider_name,i.product_name,s.renewal_ratio,s.payment_period,s.length_of_renewal")
										  ->select();
...
/***********************************************************************************************/	
	...
	/*定时任务，更新新契约品质系数表*/
	public function upCoefficient(){
		$coefficient = M("new_contract_coefficient");
		$current_time = date("Y-m-d",time());
		$result = $coefficient ->field("m_number,end_time")
							   ->select();
		$member = array();
		foreach ($result as $key => $value) {
			if ($value["end_time"] != "" && date("Y-m-d",$value["end_time"]) == $current_time) {
				$member[] = $value["m_number"];
			}
		}
		foreach ($member as $key => $value) {
			$where[$key]["m_number"] = $value;
		}
		$data = array();
		$data["input_time"] = time();
		$data["end_time"] = strtotime("+3 Months",$data["input_time"]);
		$data["new_cont_cone"] = 1;
		$result = $coefficient ->where($where)
							   ->save($data);
	}
