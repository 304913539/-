...
class MemSerFeeController extends AdminbaseController
{
    ...
    */
	public function Calculate(){
		...
		if($minfo['bzc']){
			//($minfo['bzc']);
			//首期寿险服务费(线上百分比+线下百分比)
			$initialbc = $this->initial($minfo['bzc'],$baifenbi,$post['code'],'bzc');
			//dump($initialbc);
			//续期寿险服务费
			$renewalbc = $this->renewal($minfo['bzc'],$post['code'],'bzc');
			//dump($renewalbc);
			//非寿险服务费
			$nonlifebc = $this->nonlife($minfo['bzc'],$post['code'],'bzc');
			//dump($nonlifebc);
			//管理利益manage
			$managebc = $this->manage($minfo['bzc'],$FeeCon['standard_manage'],$post['code'],'bzc');
			//dump($managebc);
			//育成利益foster
			$fosterbc = $this->foster($minfo['bzc'],$FeeCon['standard_foster']);
			//dump($fosterbc);
			//其他加扣款统计
			$otherdedubc = $this->Otherdeduc($minfo['bzc'],$post['code'],'bzc');
			//dump($otherdedubc);
			...
			//整合会员的服务费
			$bzc_save= array();
			foreach ($minfo['bzc'] as $k=>$v){
				$bzc_save[$v]['m_number'] = $v; //会员代码
				$bzc_save[$v]['initial_service'] = $initialbc[$v]['initial']; //首推服务费
				$bzc_save[$v]['renewal_service'] = $renewalbc[$v]['renewal']; //续期服务费
				$bzc_save[$v]['nonlife_service'] = $nonlifebc[$v]['nonli'];   //非寿险服务费
				$bzc_save[$v]['manage_interest'] = $managebc[$v]['manage'];   //管理利益
				$bzc_save[$v]['bred_interest'] = $fosterbc[$v]['foster'];   //育成利益
				$bzc_save[$v]['Debit'] = $otherdedubc[$v]["money"];   //其他加扣款
				$bzc_save[$v]['deduction'] = 0;   //扣税金额
				$bzc_save[$v]['already_pay'] = $nonlifebc[$v]['nonli'];   //已发工资
				$bzc_save[$v]['Should_pay'] = $initialbc[$v]['initial']+ $renewalbc[$v]['renewal']+$nonlifebc[$v]['nonli']+$managebc[$v]['manage']+$fosterbc[$v]['foster'];   //应发工资
				$bzc_save[$v]['actual_pay'] = $bzc_save[$v]['Should_pay']-$bzc_save[$v]['deduction']+$bzc_save[$v]['Debit']-$bzc_save[$v]['already_pay'];   //实发工资
				$bzc_save[$v]['on_line'] = $bzc_save[$v]['actual_pay'];   //线上发放
				$bzc_save[$v]['branch_shop'] = 1;   //分公司意见   1=>未通过 2=>通过
				$bzc_save[$v]['headquarters'] = 1;   //总公司意见 1=>未通过 2=>通过
				$bzc_save[$v]['month'] = $month;   //服务费计算时间  以月份为单位
				$bzc_save[$v]['time'] = $time;   //时间
			}
			//dump($bzc_save);
			//die;
			//数据库存储
			$otherdeducbc2 = $this->insertsql($minfo['bzc'],$post['code'],'bzc',$bzc_save);
			if($otherdeducbc2){
				$bzc = 1;
			}else{
				$bzc = 0;
		...
		if($minfo['bzj']){
			//($minfo['bzc']);
			//首期寿险服务费(线上百分比+线下百分比)
			$initialbj = $this->initial($minfo['bzj'],$baifenbi,$post['code'],'bzj');
			//续期寿险服务费
			$renewalbj = $this->renewal($minfo['bzj'],$post['code'],'bzj');
			//非寿险服务费
			$nonlifebj = $this->nonlife($minfo['bzj'],$post['code'],'bzj');
			//管理利益
			$managebj = $this->manage($minfo['bzj'],$FeeCon['standard_manage'],$post['code'],'bzj');
			//育成利益
			$fosterbj = $this->foster($minfo['bzj'],$FeeCon['standard_foster']);
			//其他加扣款统计
			$otherdedubj = $this->Otherdeduc($minfo['bzj'],$post['code'],'bzj');
			//计税（首期（线上），非寿）

			//整合会员的服务费
			$bzj_save= array();
			foreach ($minfo['bzj'] as $k=>$v){
				$bzj_save[$v]['m_number'] = $v; //会员代码
				$bzj_save[$v]['initial_service'] = $initialbj[$v]['initial']; //首推服务费
				$bzj_save[$v]['renewal_service'] = $renewalbj[$v]['renewal']; //续期服务费
				$bzj_save[$v]['nonlife_service'] = $nonlifebj[$v]['nonli'];   //非寿险服务费
				$bzj_save[$v]['manage_interest'] = $managebj[$v]['manage'];   //管理利益
				$bzj_save[$v]['bred_interest'] = $fosterbj[$v]['foster'];   //育成利益
				$bzj_save[$v]['Debit'] = $otherdedubj[$v]["money"];   //其他加扣款
				$bzj_save[$v]['deduction'] = 0;   //扣税金额
				$bzj_save[$v]['already_pay'] = $nonlifebj[$v]['nonli'];   //已发工资
				$bzj_save[$v]['Should_pay'] = $initialbj[$v]['initial']+ $renewalbj[$v]['renewal']+$nonlifebj[$v]['nonli']+$managebj[$v]['manage']+$fosterbj[$v]['foster'];   //应发工资
				$bzj_save[$v]['actual_pay'] = $bzj_save[$v]['Should_pay']-$bzj_save[$v]['deduction']+$bzj_save[$v]['Debit']-$bzj_save[$v]['already_pay'];   //实发工资
				$bzj_save[$v]['on_line'] = $bzj_save[$v]['initial_service']*($FeeCon['standard_online']/$baifenbi)-$bzj_save[$v]['deduction'];   //线上发放
				$bzj_save[$v]['off_line'] = $bzj_save[$v]['initial_service']*($FeeCon['standard_line']/$baifenbi)+$bzj_save[$v]['renewal_service']+$bzj_save[$v]['manage_interest']+$bzj_save[$v]['bred_interest']+$bzj_save[$v]['Debit'] ;   //线上发放
				$bzj_save[$v]['branch_shop'] = 1;   //分公司意见   1=>未通过 2=>通过
				$bzj_save[$v]['headquarters'] = 1;   //总公司意见 1=>未通过 2=>通过
				$bzj_save[$v]['month'] = $month;   //服务费计算时间  以月份为单位
				$bzj_save[$v]['time'] = $time;   //时间
			}

			$otherdeducbj2 = $this->insertsql($minfo['bzj'],$post['code'],'bzj',$bzj_save);
			if($otherdeducbj2){
				$bzj = 1;
			}else{
				$bzj = 0;
		...
		if($minfo['bzg']){
			//首期寿险服务费(线上百分比+线下百分比)
			$baifenbi = $FeeCon['standard_online']+$FeeCon['standard_line'];
			$initialbg = $this->initial($minfo['bzg'],$baifenbi,$post['code'],'bzg');
			//续期寿险服务费
			$renewalbg = $this->renewal($minfo['bzg'],$post['code'],'bzg');
			//非寿险服务费
			$nonlifebg = $this->nonlife($minfo['bzg'],$post['code'],'bzg');
			//管理利益(预留)
			$managebg = $this->manage($minfo['bzg'],$FeeCon['standard_manage'],$post['code'],'bzg');
			//育成利益（预留）
			$fosterbg = $this->foster($minfo['bzg'],$FeeCon['standard_foster']);
			//其他加扣款统计
			$otherdedubg = $this->Otherdeduc($minfo['bzg'],$post['code'],'bzg');
			//计税（首期（线上），非寿）

			//整合会员的服务费
			$bzg_save= array();
			foreach ($minfo['bzg'] as $k=>$v){
				$bzg_save[$v]['m_number'] = $v; //会员代码
				$bzg_save[$v]['initial_service'] = $initialbg[$v]['initial']; //首推服务费
				$bzg_save[$v]['renewal_service'] = $renewalbg[$v]['renewal']; //续期服务费
				$bzg_save[$v]['nonlife_service'] = $nonlifebg[$v]['nonli'];   //非寿险服务费
				$bzg_save[$v]['manage_interest'] = $managebg[$v]['manage'];   //管理利益(预留)
				$bzg_save[$v]['bred_interest'] = $fosterbg[$v]['foster'];   //育成利益（预留）
				$bzg_save[$v]['Debit'] = $otherdedubg[$v]["money"];   //其他加扣款
				$bzg_save[$v]['deduction'] = 0;   //扣税金额
				$bzg_save[$v]['already_pay'] = $nonlifebg[$v]['nonli'];   //已发工资
				$bzg_save[$v]['Should_pay'] = $initialbg[$v]['initial']+ $renewalbg[$v]['renewal']+$nonlifebg[$v]['nonli']+$managebg[$v]['manage']+$fosterbg[$v]['foster'];   //应发工资
				$bzg_save[$v]['actual_pay'] = $initialbg[$v]['initial']+$renewalbg[$v]['renewal']+$otherdedubg[$v]["money"]-$bzg_save[$v]['deduction'];   //实发工资
				$bzg_save[$v]['on_line'] = $bzg_save[$v]['initial_service']*($FeeCon['standard_online']/$baifenbi)-$bzg_save[$v]['deduction'];   //线上发放
				$bzg_save[$v]['off_line'] = $bzg_save[$v]['initial_service']*($FeeCon['standard_line']/$baifenbi)+$bzg_save[$v]['renewal_service']+$bzg_save[$v]['Debit'] ;   //线上发放
				$bzg_save[$v]['branch_shop'] = 1;   //分公司意见   1=>未通过 2=>通过
				$bzg_save[$v]['headquarters'] = 1;   //总公司意见 1=>未通过 2=>通过
				$bzg_save[$v]['month'] = $month;   //服务费计算时间  以月份为单位
				$bzg_save[$v]['time'] = $time;   //时间
			}

			$otherdeducbg2 = $this->insertsql($minfo['bzg'],$post['code'],'bzg',$bzg_save);
			if($otherdeducbg2){
				$bzg = 1;
			}else{
				$bzg = 0;
		...
		if($minfo['qjj']){
			//首期寿险服务费（团队）
			$initialqj = $this->initialqj($minfo['qjj'],$baifenbi,$post['code'],'qjj');
			//续期寿险服务费（个人）
			$renewalqj  = $this->renewal($minfo['qjj'],$post['code'],'qjj');
			//非寿险服务费（个人）
			$nonlifeqj = $this->nonlife($minfo['qjj'],$post['code'],'qjj');
			//其他加扣款统计
			$otherdeduqj = $this->Otherdeduc($minfo['qjj'],$post['code'],'qjj');
			//计税（非寿）

			//整合会员的服务费
			$qjj_save= array();
			foreach ($minfo['qjj'] as $k=>$v){
				$qjj_save[$v]['m_number'] = $v; //会员代码
				$qjj_save[$v]['initial_service'] = $initialqj[$v]['initial']; //首推服务费
				$qjj_save[$v]['renewal_service'] = $renewalqj[$v]['renewal']; //续期服务费
				$qjj_save[$v]['nonlife_service'] = $nonlifeqj[$v]['nonli'];   //非寿险服务费
				$qjj_save[$v]['Debit'] = $otherdeduqj[$v]["money"];   //其他加扣款
				$qjj_save[$v]['deduction'] = 0;   //扣税金额
				$qjj_save[$v]['already_pay'] = $nonlifeqj[$v]['nonli'];   //已发工资
				$qjj_save[$v]['Should_pay'] = $initialqj[$v]['initial']+ $renewalqj[$v]['renewal']+$nonlifeqj[$v]['nonli'];   //应发工资
				$qjj_save[$v]['actual_pay'] = $qjj_save[$v]['Should_pay']-$qjj_save[$v]['deduction']+$qjj_save[$v]['Debit']-$qjj_save[$v]['already_pay'];   //实发工资
				$qjj_save[$v]['off_line'] = $qjj_save[$v]['actual_pay'];   //线上发放
				$qjj_save[$v]['branch_shop'] = 1;   //分公司意见   1=>未通过 2=>通过
				$qjj_save[$v]['headquarters'] = 1;   //总公司意见 1=>未通过 2=>通过
				$qjj_save[$v]['month'] = $month;   //服务费计算时间  以月份为单位
				$qjj_save[$v]['time'] = $time;   //时间
			}
			$otherdeducqj2 = $this->insertsql($minfo['qjj'],$post['code'],'qjj',$qjj_save);
			if($otherdeducqj2){
				$qjg = 1;
			}else{
				$qjg = 0;
		...
		if($minfo['qjg']){
			//旗舰店观察期与标准店经营期相同
			//首期寿险服务费(线上百分比+线下百分比)
			$initialqg = $this->initial($minfo['qjg'],$baifenbi,$post['code'],'qjg');
			//续期寿险服务费
			$renewalqg = $this->renewal($minfo['qjg'],$post['code'],'qjg');
			//非寿险服务费
			$nonlifeqg = $this->nonlife($minfo['qjg'],$post['code'],'qjg');
			//管理利益
			$manageqg = $this->manage($minfo['qjg'],$FeeCon['standard_manage'],$post['code'],'qjg');
			//育成利益
			$fosterqg = $this->foster($minfo['qjg'],$FeeCon['standard_foster']);
			//其他加扣款统计
			$otherdeduqg = $this->Otherdeduc($minfo['qjg'],$post['code'],'qjg');
			//计税（首期（线上），非寿）

			//整合会员的服务费
			$qjg_save= array();
			foreach ($minfo['qjg'] as $k=>$v){
				$qjg_save[$v]['m_number'] = $v; //会员代码
				$qjg_save[$v]['initial_service'] = $initialqg[$v]['initial']; //首推服务费
				$qjg_save[$v]['renewal_service'] = $renewalqg[$v]['renewal']; //续期服务费
				$qjg_save[$v]['nonlife_service'] = $nonlifeqg[$v]['nonli'];   //非寿险服务费
				$qjg_save[$v]['manage_interest'] = $manageqg[$v]['manage'];   //管理利益
				$qjg_save[$v]['bred_interest'] = $fosterqg[$v]['foster'];   //育成利益
				$qjg_save[$v]['Debit'] = $otherdeduqg[$v]["money"];   //其他加扣款
				$qjg_save[$v]['deduction'] = 0;   //扣税金额
				$qjg_save[$v]['already_pay'] = $nonlifeqg[$v]['nonli'];   //已发工资
				$qjg_save[$v]['Should_pay'] = $initialqg[$v]['initial']+ $renewalqg[$v]['renewal']+$nonlifeqg[$v]['nonli']+$manageqg[$v]['manage']+$fosterqg[$v]['foster'];   //应发工资
				$qjg_save[$v]['actual_pay'] = $qjg_save[$v]['Should_pay']-$qjg_save[$v]['deduction']+$qjg_save[$v]['Debit']-$qjg_save[$v]['already_pay'];   //实发工资
				$qjg_save[$v]['on_line'] = $qjg_save[$v]['initial_service']*($FeeCon['standard_online']/$baifenbi)-$qjg_save[$v]['deduction'];   //线上发放
				$qjg_save[$v]['off_line'] = $qjg_save[$v]['initial_service']*($FeeCon['standard_line']/$baifenbi)+$qjg_save[$v]['renewal_service']+$qjg_save[$v]['manage_interest']+$qjg_save[$v]['bred_interest']+$qjg_save[$v]['Debit'] ;   //线上发放
				$qjg_save[$v]['branch_shop'] = 1;   //分公司意见   1=>未通过 2=>通过
				$qjg_save[$v]['headquarters'] = 1;   //总公司意见 1=>未通过 2=>通过
				$qjg_save[$v]['month'] = $month;   //服务费计算时间  以月份为单位
				$qjg_save[$v]['time'] = $time;   //时间
			}

			$otherdeducbj2 = $this->insertsql($minfo['qjg'],$post['code'],'qjg',$qjg_save);
			if($otherdeducbj2){
				$qjg = 1;
			}else{
				$qjg = 0;
		...
		if($qjg&&$qjj&&$qjc&&$bzg&&$bzg&&$bzc&&$uzs&&$upu){
	...
	*  作者：lzt
	*/
	public function foster($member,$ratio){
		//dump($member);
		//查询店长直推为店长的会员代码
		$ztwhere['recommend_member_id'] = array('in',$member);  //推荐人代码
		$ztwhere['status'] = 1; //会员状态 1=正常 0=禁用  -1=删除
		$ztwhere['member_level'] = 2; //0=>普通会员  1=>资深业务员 2=>标准店店长  3=>旗舰店店长
		$ztmember = $this->member->where($ztwhere)->field('recommend_member_id,m_number')->select();
	//	dump($ztmember);

		$ztmember2=array();
		foreach ($ztmember as $ztk=>$ztv){
			$ztmember2[] = $ztv['m_number'];
		}
		//dump($ztmember2);
		//查询新契约品质系数
	...
	//	dump($contract);
		//查询直推为店长会员的首期保单
		if($ztmember2){
			$foster = $this->GetValPre($ztmember2,2);
		}
	//	dump($foster);
//die;
		//分类给每个店长并统计保单累计价值保费
		$count =array();
		foreach ($ztmember as $ztk2=>$ztv2){
			foreach ($foster as $k=>$v){
				if($ztv2['m_number'] == $v['standard_shop_number']){
					if(!isset($count[$ztv2['recommend_member_id']])){
						$count[$ztv2['recommend_member_id']]['number'] = $ztv2['recommend_member_id']; //直推店长代码
						$count[$ztv2['recommend_member_id']]['premium'] = floatval($v['value_premium']); //累计有效价值保费;
						$count[$ztv2['recommend_member_id']]['contract'] = floatval($contract[$ztv2['recommend_member_id']]); //新契约品质系数
					}else{
						$count[$ztv2['recommend_member_id']]['premium'] += $v['value_premium']; //累计有效价值保费
					}
				}
			}
		}
	...
	//	die;
		//计算业务员首期寿险服务费
		//dump($ratio);
		$foster_service = array();
		foreach ($count as $k=>$v){
			$foster_service[$k]['number'] =$v['number'];
			$foster_service[$k]['foster'] =$v['premium']*$v['contract']*$ratio;
		}
		//dump($foster_service);
		return $foster_service;
		//计算育成利益
	}
	/*
	*  功能：管理利益
...
//		dump($member);
		...
		$contract = M('new_contract_coefficient')->where($where)->getField('m_number,new_cont_cone',true);
		//判断缓存是否存在
//		$life = "manage_service_".$code."_".$type;
//		if(S($life)){
//			$manage = S($life);
//		}else{
			//查询店长旗下所有有效保单的价值保费
			$manage = $this->GetValPre($member,2);
		...
		foreach ($count as $k=>$v){
			...
			$manage_service[$k]['manage'] =$v['premium']*$v['contract']*$ratio;
		}
	...
	*/
	...
	//	dump($save);
		//查询业务员所属店铺
		$mwhere['m_number'] = array('in',$member);
		$mshu = $this->member->where($mwhere)->getField('m_number,shop_number,flagship_number,branch_shop_number',true);
		//dump($mshu);
		//dump($save);
		$save2 =array();
		foreach ($save as $k =>$v){
			$save2[$k] = $v;
			$save2[$k]['shop_number'] = $mshu[$k]['shop_number'];//所属标准店代码
			$save2[$k]['flagship_number'] = $mshu[$k]['flagship_number'];//所属旗舰店代码
			$save2[$k]['branch_shop_number'] = $mshu[$k]['branch_shop_number'];//所属分公司代码
		}
		$life = "service_fee_".$code."_".$type;
		S($life,$save2,8440);
		//dump($save2);
		//die;
		//服务费计算表插入数据
...
//		dump($ance_re);
    ...
    *  作者：lzt
    */
	public function initialqj($member,$ratio,$code,$type){
		//查询新契约品质系数
		$where['m_number'] = array('in',$member);
		$contract = M('new_contract_coefficient')->where($where)->getField('m_number,new_cont_cone',true);

		//查询旗舰店所有有效价值保费
		$insurance = $this->GetValPre($member,3);
		//组装数组
		$count =array();
		foreach ($insurance as $k=>$v){
			if(!isset($count[$v['flag_shop_number']])){
				$count[$v['flag_shop_number']]['number'] = $v['flag_shop_number']; //业务员代码
				$count[$v['flag_shop_number']]['premium'] = floatval($v['value_premium']); //累计有效价值保费;
				$count[$v['flag_shop_number']]['contract'] = floatval($contract[$v['flag_shop_number']]); //新契约品质系数
			}else{
				$count[$v['flag_shop_number']]['premium'] += $v['value_premium']; //累计有效价值保费
			}
		}
		//计算业务员首期寿险服务费
		$initial_service = array();
		...
		//所辖标准点累计基本法利益（含预留利益）
		//标准店筹备期
		$lifebc = "service_fee_".$code."_bzc";
		$bzc = S($lifebc);
		if($bzc){
			$bzc2 = array();
			foreach ($bzc as $k=>$v ){
				$bzc2[$v['flagship_number']] += $v['initial_service']+$v['renewal_service']+$v['manage_interest']+$v['bred_interest'];
			}
		}
		//标准店经营期
		$lifebj = "service_fee_".$code."_bzj";
		$bzj = S($lifebj);
		if($bzj){
			$bzj2 = array();
			foreach ($bzj as $k=>$v ){
				$bzj2[$v['flagship_number']] += $v['initial_service']+$v['renewal_service']+$v['manage_interest']+$v['bred_interest'];
			}
		}
		//标准店观察期
		$lifebg = "service_fee_".$code."_bzg";
		$bzg = S($lifebg);
		if($bzg){
			$bzg2 = array();
			foreach ($bzj as $k=>$v ){
				$bzg2[$v['flagship_number']] += $v['initial_service']+$v['renewal_service']+$v['manage_interest']+$v['bred_interest'];
			}
		}
		//所辖业务员累计基本法利益
		//普通业务员
		$lifeut = "service_fee_".$code."_upt";
		$upt = S($lifeut);
		if($upt){
			$upt2 = array();
			foreach ($bzj as $k=>$v ){
				$upt2[$v['flagship_number']] += $v['initial_service']+$v['renewal_service'];
			}
		}
		//资深业务员
		$lifeus = "service_fee_".$code."_uzs";
		$uzs = S($lifeus);
		if($uzs){
			$uzs2 = array();
			foreach ($bzj as $k=>$v ){
				$uzs2[$v['flagship_number']] += $v['initial_service']+$v['renewal_service'];
			}
		}
		die;
...
//		[3] => string(10) "7526170516"
		...
		$data['salesman_number'] = '7522170516';  //业务员代码
		$data['policy_number'] = '868686';  //保单号
		$data['policy_status'] = '1';  //保单状态	1=>有效	2=>失效
		$data['insurance_type'] = '1';  //保单类型 1=>寿险  2=>非寿险(不含车险)  3=>车险
...
//		[3] => string(10) "7526170516"
		...
		$data['policy_status'] = '0';  //保单状态	0=>有效	1=>无效
		$data['insurance_money'] = '1000';  //保额
		$data['insurance_premium'] = '100';  //规模保费
		$data['insured_date'] = '1492531200';  //承保日期 3.11
