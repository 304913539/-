...
class PersonalPerformanceController extends AdminbaseController
{
	...
	public function get_result(){
		...
		//获得$result
		foreach ($result as $key => $value) {
			if ($value["recommend_member_id"] == 0) {
				$result[$key]["recommend_member_id"] = "无";
			}

			if ($value["shop_number"] == 0) {
				$result[$key]["shop_number"] = "无";
			}else{
				$re = $organization ->where("org_code = ".$value["shop_number"])->field("name")->find();
				$result[$key]["shop_number"] = $re["name"];
			}

			 if ($value["flagship_number"] == 0) {
				$result[$key]["flagship_number"] = "无";
			}else{
				$re = $organization ->where("org_code = ".$value["flagship_number"])->field("name")->find();
				$result[$key]["flagship_number"] = $re["name"];
			}

			 if ($value["branch_shop_number"] == 0) {
				$result["branch_shop_number"] = "无";
			}else{
				$re = $organization ->where("org_code = ".$value["branch_shop_number"])->field("name")->find();
				$result[$key]["branch_shop_number"] = $re["name"];
			}
	...
	public function personal(){
		...
		/*将本月寿险，本季度寿险，本半年寿险和本年寿险合并，将可能为空的寿险结果赋值为0*/
		...
		/*将本月车险，本季度车险，本半年车险和本年车险合并，将可能为空的车险结果赋值为0*/
		...
		/*将本月非寿险，本季度非寿险，本半年非寿险，本年非寿险合并，将可能为空的结果赋值为0*/
