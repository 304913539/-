	...
	public function index(){
        ...
        $where['_string'].= ' AND insured_date<='.$endThisMonth;
         ...
         foreach ($result as $key => $value) {
			...
			$result[$key]['two_state']      = $value['two_state']==1?'成功':'失败';
			$result[$key]['three_state']    = $value['two_state']==1?'成功':'失败';
			$result[$key]['four_state']     = $value['two_state']==1?'成功':'失败';
			$result[$key]['five_state']     = $value['two_state']==1?'成功':'失败'; 
...
//		//合并单元格  
    ...
    ->setBold(true);
        ...
        $a=4;
				...
				->setCellValue('AL'.$a, '=IFERROR(AJ'.$a.'/Ak'.$a.',"")');
//				合并单元格
...
//		----------------------------------------应收清单
		...
		$objPHPExcel->getActiveSheet(4)->setCellValue('A1', '应收清单');
		//合并单元格  
    	$objPHPExcel->getActiveSheet(4)->mergeCells('A1:V1');
	//设置居中  
    $objPHPExcel->getActiveSheet(4)->getStyle('A1:V1')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	//字体加粗  
    $objPHPExcel->getActiveSheet(4)->getStyle('A1')->getFont()->setName('宋体') //字体  
    ->setBold(true); //字体加粗
        $objPHPExcel ->setActiveSheetIndex(4)
       				 ->setCellValue('A2', "序号")//设置列的值
                     ->setCellValue('B2', "分公司")//设置列的值
                     ->setCellValue('C2', "保单号")//设置列的值
                     ->setCellValue('D2', "投保人姓名")//设置列的值
                     ->setCellValue('E2', "投保人电话")//设置列的值
                     ->setCellValue('F2', "供应商")//设置列的值
                     ->setCellValue('G2', "服务人员代码")//设置列的值
                     ->setCellValue('H2', "服务人员")//设置列的值
                     ->setCellValue('I2', "部门代码")//设置列的值*/
                     ->setCellValue('J2', "分区代码")//设置列的值*/
                     ->setCellValue('K2', "应收规保")//设置列的值*/
                     ->setCellValue('L2', "实收规保")//设置列的值*/
                     ->setCellValue('M2', "缴费日期")//设置列的值*/
                     ->setCellValue('N2', "缴费次数")//设置列的值*/
                     ->setCellValue('O2', "二次成功")//设置列的值
                     ->setCellValue('P2', "二次成功日期")//设置列的值
                     ->setCellValue('Q2', "三次成功")//设置列的值
                     ->setCellValue('R2', "三次成功日期")//设置列的值*/
                     ->setCellValue('S2', "四次成功")//设置列的值*/
                     ->setCellValue('T2', "四次成功日期")//设置列的值*/
                     ->setCellValue('U2', "五次成功")//设置列的值*/
                     ->setCellValue('V2', "五次成功日期");//设置列的值*/
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('A')->setWidth(10);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('B')->setWidth(15);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('C')->setWidth(15);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('D')->setWidth(12);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('E')->setWidth(12);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('F')->setWidth(12);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('G')->setWidth(12);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('H')->setWidth(12);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('I')->setWidth(12);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('J')->setWidth(12);
        $objPHPExcel->getActiveSheet(4)->getColumnDimension('K')->setWidth(12);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('L')->setWidth(12);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('M')->setWidth(15);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('N')->setWidth(12);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('O')->setWidth(12);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('P')->setWidth(15);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('Q')->setWidth(12);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('R')->setWidth(15);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('S')->setWidth(12);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('T')->setWidth(15);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('U')->setWidth(12);
		$objPHPExcel->getActiveSheet(4)->getColumnDimension('V')->setWidth(15);
		$arr4=$this->getonelist();
        $a=3;
		$i=1;
		foreach ($arr4 as $key => $value) {
           				 $objPHPExcel->setActiveSheetIndex(4)
                        ->setCellValue('A'.$a, $i)
                        ->setCellValue('B'.$a, $value['branch_shop_number'])
                        ->setCellValue('C'.$a, $value['policy_number'])
                        ->setCellValue('D'.$a, $value['policy_holder_name'])
                        ->setCellValue('E'.$a, $value['policy_holder_telephone'])
                        ->setCellValue('F'.$a, $value['provider_name'])
                        ->setCellValue('G'.$a, $value['salesman_number'])
                        ->setCellValue('H'.$a, $value['salesman_name'])
                        ->setCellValue('I'.$a, $value['standard_shop_number'])
                        ->setCellValue('J'.$a, $value['flag_shop_number'])
                        ->setCellValue('K'.$a, $value['insurance_premium'])
                        ->setCellValue('L'.$a, $value['real_insurance_premium'])
                        ->setCellValue('M'.$a, $value['insured_date'])
                        ->setCellValue('N'.$a, $value['renew_count'])
                        ->setCellValue('O'.$a, $value['two_state'])
						->setCellValue('P'.$a, $value['two_time'])
						->setCellValue('Q'.$a, $value['three_state'])
						->setCellValue('R'.$a, $value['three_time'])
						->setCellValue('S'.$a, $value['four_state'])
						->setCellValue('T'.$a, $value['four_time'])
						->setCellValue('U'.$a, $value['five_state'])
						->setCellValue('V'.$a, $value['five_time']);
            $a++;
			$i++;
        }
        $objPHPExcel->getActiveSheet(4)->setTitle('应收清单');
		
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); 
        header('Content-Disposition:attachment;filename="保单.xlsx"');
...
//		第五次缴费承保日期
        ...
        $endThisMonth = mktime(0, 0, 0, $starmonth+3, 1, $staryear);
		
		$startime1 = mktime(0, 0, 0, $starmonth, 1, $staryear+1);
        $endThisMonth1 = mktime(0, 0, 0, $starmonth+3, 1, $staryear+1);
     	
		$startime2 = mktime(0, 0, 0, $starmonth, 1, $staryear+2);
        $endThisMonth2 = mktime(0, 0, 0, $starmonth+3, 1, $staryear+2);
		
		$startime3 = mktime(0, 0, 0, $starmonth, 1, $staryear+3);
        $endThisMonth3 = mktime(0, 0, 0, $starmonth+3, 1, $staryear+3);
		
//		p(date('Y-m-d H:i:s',$endThisMonth2)); 
		$startime4 = mktime(0, 0, 0, $starmonth, 1, $staryear+4);
        $endThisMonth4 = mktime(0, 0, 0, $starmonth+3, 1, $staryear+4);
        
        //查询判断
        $where['insured_date'] = array(array(array('gt',$startime),array('lt',$endThisMonth)),array(array('gt',$startime1),array('lt',$endThisMonth1)),array(array('gt',$startime2),array('lt',$endThisMonth2)),array(array('gt',$startime3),array('lt',$endThisMonth3)),array(array('gt',$startime4),array('lt',$endThisMonth4)),'or');
		$map['_complex'] = $where;
		$map['policy_status']  = array('eq',0);
         $insurance = M('insurance_re');
         $result = $insurance -> join('LEFT JOIN renew  on insurance_re.policy_number = renew.insurance_num')
                              -> where($map)
                              -> order("insurance_re.id desc")
